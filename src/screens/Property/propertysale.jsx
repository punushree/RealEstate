...
const PropertySale = () => {
  ...
  let saledata = propData.filter(function (obj, index) {
    return obj.purpose === "for-sale";
  });
  ...
  const loaddefault = async () => {
    ...
    setLoading(false);
    console.log("Defult sale*********" + JSON.stringify(result));
  };
  const handlesubmit = async () => {
    ...
    let finalData = saledata.filter(function (obj, index) {
      let menu =
        obj.location.some(({ externalID }) => externalID === loction) &&
        obj.category.some(({ externalID }) => externalID === type);
      return menu;
    });
    setPropertyData(finalData);
    console.log("finalsale*********" + JSON.stringify(finalData));
  };

  const toComponentB = (external) => {
    console.log("typepass" + type);
    navigate("/single", {
      state: { externalID: external, data: propertyData, fortype: "sale" },
    });
  };
        ...
        <div className="row gx-5">
          <div className="col-8">
            <div className="p-4  data">
              <OwlCarousel className="owl-theme" {...optionsc}>
                {propertyData?.map((property) => (
                  <div className="card h-100">
                    ...
                    <div className="item-image">
                      <img
                        src={property.coverPhoto ? property.coverPhoto.url : ""}
                        width={300}
                        height={260}
                    ...
                    <div className="card-body">
                      <div className="item-content">
                        ...
                        <div className="item-price ">
                          {" "}
                          <span>bedroom | {property.rooms}</span>{" "}
                      ...
                      <div style={{ color: "#dc3545" }}>
                        ...
                        <center>
                          <a
                            onClick={() => {
                              toComponentB(property.externalID);
                            }}
                          >
                            {locale.label.viewDetail}
                          </a>
                        </center>
                      </div>
                    </div>
          ...
          <div className="col-4">
            <div className="p-2 border filter">
              ...
              <div class="mb-3">
                <label for="locatagoryc" class="form-label">
                  {locale.label.selectCategory}
                </label>
                <select
                  id="catagory"
                  value={type}
                  ...
                  className="form-control"
                >
                  <option value="4">{locale.label.apartment}</option>
                  <option value="3">{locale.label.villa}</option>
                  <option value="1">{locale.label.residentialBuilding}</option>
                </select>
              </div>
              <div class="mb-3 item-area ">
                {locale.label.selectPrice}
                <Slider
                  id="range-slider"
                  style={{ color: "#dc3545" }}
                  value={price}
              ...
              <div className="item-price mb-3" style={{ fontSize: "10px" }}>
                {" "}
                <span>{price[0]} </span> <span> {price[1]}</span>{" "}
              </div>
              <div class="mb-3 item-area">
                {locale.label.selectArea}
                <Slider
                  id="area-slider"
                  style={{ color: "#dc3545" }}
                  value={area}
              ...
              <div className="item-price mb-3" style={{ fontSize: "10px" }}>
                {" "}
                <span>{area[0]} </span> <span> {area[1]}</span>{" "}
              </div>

              <div class="mb-3 " style={{ alignItems: "center" }}>
                {locale.label.bedrooms}
                <button className="btn mx-3" onClick={handleDecrement}>
                  <i className="fa fa-minus-square plusminus"></i>
                </button>
                {bedroom}
                <button className="btn mx-3" onClick={handleIncrement}>
                  <i className="fa fa-plus-square plusminus"></i>
                </button>
              </div>
              <div class="mb-3 ">
                {locale.label.bathroom}
                <button className="btn mx-3" onClick={handlebathDecrement}>
                  <i className="fa fa-minus-square plusminus"></i>
                </button>
                {bathroom}
                <button className="btn mx-3" onClick={handlebathIncrement}>
                  <i className="fa fa-plus-square plusminus"></i>
                </button>
              </div>
